...
* Get Bladenight FLutter Project from Git
    * Setup firebase https://firebase.google.com/docs/crashlytics/get-started?platform=flutter&hl=de
        ...
        * add key.properties to android project - see android doc
          ```
          storePassword=your_storepwd
          keyPassword=your_keypwd
          keyAlias=upload
...
* Build in IntelliJ Terminal. Enter the following:
  ...
  flutter clean
  ```
* for Android Release PlayStore build
  ```bash
  * flutter build appbundle --no-tree-shake-icons
  ``` 
    * If problems with gradle
  ```bash
  flutter build appbundle --verbose
  ```
  *Build apk for install outside google
  ```bash
  flutter build appbundle --verbose
  ```
...
* for iOS
  ...
  ```bash
   ...
   flutter build ios
  ```
    * Building for XCode doesn't work if Pod not in sync
    * do following before build with Xcode
  ```bash
    cd ios
    pod update
    pod install
...
* missing local.properties
  ...
  flutter.versionName=1.25.08
  ```

### Cant't run on iPhone. message: Martina Huth, Ines Huth, Fabienne Huth Martina Huth, Ines Huth, Fabienne Huth „iproxy“ kann nicht geöffnet werden, da der Entwickler nicht verifiziert werden kann.
