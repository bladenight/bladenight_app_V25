...
${KEYTOOLCOMMAND} -importkeystore -srckeystore ${FILENAME}.pkcs12 -srcstoretype PKCS12 -destkeystore ${CERTPATH} -storepass ${PASSWORD} -deststorepass ${PASSWORD} -srcstorepass ${PASSWORD} -noprompt
cp ${FILENAME}.pkcs12 $CERTPATH/${FILENAME}.pkcs12

printf "${RED}Use assets/certs/$FILENAME.pkcs12 for server and keep password and other files in ${CERTPATH} and keys on a safe place\n"