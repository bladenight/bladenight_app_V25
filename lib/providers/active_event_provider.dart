...
class ActiveEventProvider with ChangeNotifier {
        ...
        if (kIsWeb || compareResult != 0) {
          if (rpcEvent.nodes == [] && rpcEvent.status != EventStatus.noevent) {
            ...
            var rn = await RoutePoints.getActiveRoutePointsByNameWamp(
                rpcEvent.routeName);
            if (rn.points.isNotEmpty) {
              var routeLength = GeoLocationHelper.calculateDistance(rn.points);
              _event = rpcEvent.copyWith(
                  nodes: rn.points, routeLength: routeLength.toInt());
            }
          } else {
            if (rpcEvent.routeLength == 0 && rpcEvent.nodes.length > 2) {
              var routeLength =
                  GeoLocationHelper.calculateDistance(rpcEvent.nodes);
              _event = rpcEvent.copyWith(routeLength: routeLength.toInt());
            } else {
              _event = rpcEvent;
            }
          }
  ...
  void initOrUpdateSettingsHeadlessSettings() async {
    try {
      ...
      ///TODO switch this to hive isolated
