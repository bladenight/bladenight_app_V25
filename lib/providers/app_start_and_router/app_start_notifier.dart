...
class AppStartNotifier extends _$AppStartNotifier {
  ...
  void initCrashLogs() async {
    ...
    BnLog.info(text: 'BladenightApp was started');
  ...
  Future<void> migrateFriendsToHiveDB() async {
    if (!kIsWeb && HiveSettingsDB.firstStart2508) {
      final prefs = await SharedPreferences.getInstance();
      final String? jsonData = prefs.getString('friendlist');
      int maxFriendId = -1;
      if (jsonData != null) {
        var friends = MapperContainer.globals.fromJson<List<Friend>>(jsonData);
        FriendsDb.saveFriendsAsync(friends);
        for (var friend in friends) {
          maxFriendId = math.max(friend.friendId, maxFriendId);
        }
      }

      int finalFriendId = -1;
      int? id = prefs.getInt('friendid');
      if (id == null || id == 0) {
        //not set
        finalFriendId = 1;
      } else {
      ...
      await FriendsDb.saveFriendId(math.max(finalFriendId, maxFriendId));
    }

    //PreferencesHelper.getImagesAndLinksPref();
    HiveSettingsDB.setFirstStart2508(false);
  }
}
