...
class _MapButtonsOverlay extends ConsumerState<MapButtonsLayer>
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Stack(fit: StackFit.passthrough, children: [
        ...
        //Right side buttons
        //#######################################################################
        Positioned(right: 70, bottom: 30, child: Scalebar()),
        Positioned(
          right: 10,
          bottom: 40,
        ...
        if (orientationPortrait) ...[
          ...
          PositionedVisibilityOpacity(
            ...
            onPressed: () {
              ...
              _mapControllerMove(
                  controller, controller.camera.center, camera.zoom - 0.5);
            },
            visibility: ref.watch(mapMenuVisibleProvider),
            child: Icon(
  ...
  Future<bool> _stopLocationService() async {
    MapController controller = MapController.of(context);
    _mapControllerMove(controller, defaultLatLng, defaultInitialZoom);
    ref.read(headingMarkerAmountProvider.notifier).setSize(defaultInitialZoom);
  ...
  void startFollowingTrainHead(MapController controller) {
    ...
    _mapControllerMove(controller, defaultLatLng, controller.camera.zoom);
    ref
        .read(headingMarkerAmountProvider.notifier)
        .setSize(controller.camera.zoom);
    locationSubscription = LocationProvider().trainHeadUpdateStream.listen(
      (value) {
        _mapControllerMove(controller, value, controller.camera.zoom);
      },
    );
  ...
  void _mapControllerMove(
      MapController controller, LatLng latLng, double zoom) {
    controller.move(latLng, zoom);
    ref
        .read(headingMarkerAmountProvider.notifier)
        .setSize(controller.camera.zoom);
  }
