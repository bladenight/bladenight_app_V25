...
class GeoLocationHelper {
  /// calculate heading marker for route evenly distributed
  /// over the route
  /// to increase calculation put route length to
  static List<HeadingPoint> calculateHeadings(List<LatLng> routePoints,
      {int routeLength = 0, zoom = defaultInitialZoom}) {
    if (routePoints.isEmpty) {
      return [];
    }
    List<HeadingPoint> headingPoints = <HeadingPoint>[];
    if (routeLength == 0) {
      routeLength = calculateDistance(routePoints).toInt();
    }
    //max 20
    // zoom <11 fix
    ...
    var meanDistance = routeLength / headingsCount;
    if (routeLength < 100) {
      meanDistance = 50;
      headingsCount = 2;
    }
    ...
    double totalSum = 0;
    double nextHeadingPoint = meanDistance;
    ...
    for (var i = 0; i < count - 2; i++) {
      ...
      if (totalSum + distP1P2 < nextHeadingPoint) {
        totalSum = totalSum + distP1P2;
        continue;
      } else {
        var missingDist = totalSum + distP1P2 - nextHeadingPoint;
        var heading = mp.SphericalUtil.computeAngleBetween(point1, point2);
        var bearing = bearingBetween(point1.latitude, point1.longitude,
            point2.latitude, point2.longitude);
        var newPoint = moveLatLng(
            LatLng(point1.latitude, point1.longitude), missingDist, bearing);
        headingPoints.add(HeadingPoint(
            LatLng(newPoint.latitude, newPoint.longitude),
            heading.toDouble(),
            radians(bearing),
            distP1P2.toDouble()));
        nextHeadingPoint = nextHeadingPoint + meanDistance;
      }
    }
